...
our %return_counts = (
        'getDatasets' => 1,
        'buildNetwork' => 1,
        'buildFirstNeighborNetwork' => 1,
        'buildInternalNetwork' => 1,
        'version' => 1,
);
...
sub call_method {
    ...
    my $ctx = KBaseNetworkServerContext->new(client_ip => $self->_plack_req->address);
    
    my $args = $data->{arguments};

        # Service KBaseNetwork does not require authentication.
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service KBaseNetwork.");
    }
...
package KBaseNetworkServerContext;

use strict;

=head1 NAME

KBaseNetworkServerContext
