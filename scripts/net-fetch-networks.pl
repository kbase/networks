...
#This API is for demo use only. A standard one will take place by the end Jan, 20
#example:
#echo 'kb|g.3899.locus.139'|perl /kb/dev_container/modules/networks/scripts/net_fetch_networks.pl 'kb|g.3899'

my $url       = "http://kbase.us/services/networks";
my $usage = "Usage: echo genelist |net_fetch_networks 'kb|g.3899' 'GENE_GENE'\n";
my $help       = 0;
my $version    = 0;
...
if($help){
print "This command is used to query the first neighbor networks for a list of genes.\nThe genes should be KBase gene loucs ID\n";
print "We suggest submit one gene at a time  due to a large collection of network datasets\n";
print "\nUsage:\n";
print "echo genelist |net_fetch_networks 'kb|g.3899 'GENE_GENE''\n\n";
print "genes is a list of genes, such as:'kb|g.3899.locus.534' 'kb|g.3899.locus.19668' 'kb|g.3899.locus.6286' 'kb|g.3899.locus.13048' 'kb|g.3899.locus.23790'\n";
...
if($version)
{
    print "net_fetch_network version 1.0\n";
