...
=head1 buildInternalNetworkLimitedByStrength

Example:

    buildInternalNetworkLimitedByStrength [arguments] < input > output
...
This script is a wrapper for the CDMI-API call buildInternalNetworkLimitedByStrength. It is documented as follows:
...
$geneIds is a reference to a list where each element is a string
...
my $usage = "Usage: $0 [--host=140.221.92.222:7064] dataset_ids edge_types cut_off < ids\n";
